...
function updatePlayer(dt){
  ...
  if(mountedHorse){
    ...
    if(move.lengthSq()>0){
      ...
      data.rideSpeed+=(ridingSpeed-data.rideSpeed)*Math.min(1,dt*3.6);
      ...
      data.walkTime+=dt*(data.rideGrounded?9.0:5.0);
      const swing=Math.sin(data.walkTime)*(data.rideGrounded?0.48:0.22), legs=data.model.legs||[];
  ...
  if(move.lengthSq() > 0) move.multiplyScalar(moveSpeed * dt);
